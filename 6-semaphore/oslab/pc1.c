...
/* sem_t* api_sem_open(char* name, int value);

int sem_wait(sem_t* sem);

int sem_post(sem_t* sem);
int sem_unlink(char* sem);
...
int main(void)
{
 ...
 /*    // // 创建信号量 O_CREAT | O_EXCL, 0644,
    ...
    // 创建信号量 O_CREAT | O_EXCL, 0644, */
    empty = api_sem_open("empty", 9);
    if(empty == -1/* SEM_FAILED */)
    {
      printf("errno: %s\n",strerror(errno));
    }

    full = api_sem_open("full", 0);
    if(full == -1/* SEM_FAILED */)
    {
      printf("errno: %s\n",strerror(errno));
    }
    mutex = api_sem_open("mutex", 1);
    if(mutex == -1/* SEM_FAILED */)
    {
       printf("errno: %s\n",strerror(errno));
    }
